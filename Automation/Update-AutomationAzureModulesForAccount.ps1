...
$script:AzureRMContainerInstance="AzureRM.ContainerInstance"
$script:AzureRMContainerRegistry="AzureRM.ContainerRegistry"
...
function Update-ProfileAndAutomationVersionToLatest {
    ...
    # Download AzureRM.ContainerRegistry to temp location
    $ModuleContentUrl = Get-ModuleContentUrl $script:AzureRMContainerRegistry
    $CRURL = (Invoke-WebRequest -Uri $ModuleContentUrl -MaximumRedirection 0 -UseBasicParsing -ErrorAction Ignore).Headers.Location
    $CRPath = Join-Path $env:TEMP ($script:AzureRMContainerRegistry + ".zip")
    $WebClient.DownloadFile($CRURL, $CRPath)

    # Download AzureRM.ContainerInstance to temp location
    $ModuleContentUrl = Get-ModuleContentUrl $script:AzureRMContainerInstance
    $CIURL = (Invoke-WebRequest -Uri $ModuleContentUrl -MaximumRedirection 0 -UseBasicParsing -ErrorAction Ignore).Headers.Location
    $CIPath = Join-Path $env:TEMP ($script:AzureRMContainerInstance + ".zip")
    $WebClient.DownloadFile($CIURL, $CIPath)
    ...
    $CIUnzipPath = Join-Path $PathFolder $script:AzureRMContainerInstance
    Expand-Archive -Path $CIPath -DestinationPath $CIUnzipPath -Force
    $CRUnzipPath = Join-Path $PathFolder $script:AzureRMContainerRegistry
    Expand-Archive -Path $CRPath -DestinationPath $CRUnzipPath -Force
    ...
    Import-Module (Join-Path $CRUnzipPath "AzureRM.ContainerRegistry.psd1") -Force -Verbose
    Import-Module (Join-Path $CIUnzipPath "AzureRM.ContainerInstance.psd1") -Force -Verbose
}
